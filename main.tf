...
resource "aws_security_group" "allow_all_vpc" {
  description = "All traffic in the VPC - Managed by Terraform for ${var.owner}"
...
resource "aws_security_group" "external_connectivity" {
  description = "External Connectivity Security Group - Managed by Terraform for ${var.owner}"
  ...
  # schema-registry http interface - only accessible on host, without this
  # c3 needs access
  ingress {
      from_port = 8081
      to_port = 8081
  ...
  # control-center http interface - only accessible on host, without this
  # c3 needs access
  ingress {
      from_port = 9021
      to_port = 9021
...
resource "aws_instance" "broker" {
  ...
  security_groups = [
    "${aws_security_group.external_connectivity.name}",
    "${aws_security_group.allow_all_vpc.name}"
  ]
  root_block_device {
  ...
  tags ="${merge(
    map("Name", "${var.prefix}-kafka-broker-${count.index}"),
    local.common_tags,
    var.broker_tags
    )}"
}
...
resource "aws_instance" "worker" {
  ...
  tags ="${merge(
    map("Name", "${var.prefix}-worker-node-${count.index}"),
    local.common_tags,
    var.worker_tags
    )}"
}
